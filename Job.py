...
class Job:
    ...
    def f_insert(self):
        if "None" in [self.e1.get(),self.e2.get(),self.e3.get(),self.v6.get()]:
            ...
            try:
                c.execute(f'insert into Job values ("{int(self.e1.get())}","{self.e2.get()}","{self.e3.get()}","{int(self.v6.get())}");')
                db.commit()
                self.root1.destroy()
    ...
    def com_delete(self):
        self.root2=Tk()
        self.root2.title('Job Deletion')
        self.root2.geometry('300x150')
        
        self.l1=Label(self.root2,text="Job Application no.")
        self.l1.grid(row=0,column=0)
        self.v4=StringVar(self.root2,value="None")
        self.list1=[]
        c.execute(f"select * from Job")
    ...
    def f_delete(self):
        if self.v4.get() == "None":
               messagebox.showerror("Error","Job Application no. not selected")
        else:
            try:
                  c.execute(f'DELETE FROM Job WHERE Japplication= "{int(self.v4.get())}";')
                  db.commit()
                  self.root2.destroy()
    ...
    def com1_update(self):
        self.root3=Tk()
        self.root3.title('Job Updation')
        self.root3.geometry('300x150')

        self.l1=Label(self.root3,text="Job Application no.")
        self.l1.grid(row=0,column=0)
    ...
    def com2_update(self):
        self.root3.destroy()
        self.root4=Tk()
        self.root4.title('Job Updation')
        self.root4.geometry('300x150')
                    
        self.l2=Label(self.root4,text="Job Description ")
        self.l2.grid(row=1,column=0)
        self.e2s=Entry(self.root4,borderwidth=5)
        self.e2s.grid(row=1,column=1)
                    
        self.l3=Label(self.root4,text="Company Role ")
        self.l3.grid(row=2,column=0)
        ...
        c.execute(f"SELECT * FROM Job WHERE Japplication = {int(self.v5.get())}")
        records=c.fetchall()
    ...
    def f_update(self):
         c.execute(f'UPDATE Job SET Jdescription = "{self.e2s.get()}",Jrole = "{self.e3s.get()}" ,Cpregno="{self.v7.get()}" WHERE Japplication= "{int(self.v5.get())}";' )
         db.commit()
         self.root4.destroy()
    ...
    def com_show(self):
        self.root5=Tk()
        self.root5.title("Job Database")
        self.root5.geometry('300x150')

        self.l8=Label(self.root5,text="Japplication  ")
        self.l8.grid(row=0,column=0)
        self.l9=Label(self.root5,text="Jdescription  ")
        self.l9.grid(row=0,column=1)
        self.l10=Label(self.root5,text="Jrole  ")
