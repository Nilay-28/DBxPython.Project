...
class Company:
    ...
    def f_insert(self):
        if "None" in [self.e1.get(),self.e2.get(),self.e3.get()]:
            messagebox.showerror("Error","All important fields not filled")
        else:
            try:
                c.execute(f'insert into Company values ("{int(self.e1.get())}","{self.e2.get()}","{self.e3.get()}");')
                db.commit()
    ...
    def com_delete(self):
        self.root2=Tk()
        self.root2.title('Company Deletion')
        self.root2.geometry('300x150')
        
        self.l1=Label(self.root2,text=" Company RegNo")
        self.l1.grid(row=0,column=0)
    ...
    def f_delete(self):
        if self.v4.get() == "None":
               messagebox.showerror("Error","Company Regno not selected")
        else:
            try:
                c.execute(f'DELETE FROM Company WHERE Cpregno= "{int(self.v4.get())}";')
                db.commit()
                self.root2.destroy()
    ...
    def com1_update(self):
        self.root3=Tk()
        self.root3.title('Company Updation')
        self.root3.geometry('300x150')

        self.l1=Label(self.root3,text=" Company RegNo")
        self.l1.grid(row=0,column=0)
    ...
    def com2_update(self):
        self.root3.destroy()
        self.root4=Tk()
        self.root4.title('Company Updation')
        self.root4.geometry('300x150')
                    
        self.l2=Label(self.root4,text="Company Name ")
        self.l2.grid(row=1,column=0)
        self.e2s=Entry(self.root4,borderwidth=5)
        self.e2s.grid(row=1,column=1)
                    
        self.l3=Label(self.root4,text="Company Domain ")
        self.l3.grid(row=2,column=0)
        self.e3s=Entry(self.root4,borderwidth=5)
        self.e3s.grid(row=2,column=1)

        c.execute(f"SELECT * FROM company WHERE Cpregno = {int(self.v5.get())}")
        records=c.fetchall()
    ...
    def f_update(self):
         c.execute(f'UPDATE Company SET Cpname = "{self.e2s.get()}",CpDomain = "{self.e3s.get()}" WHERE Cpregno= "{int(self.v5.get())}";' )
         db.commit()
         self.root4.destroy()
    ...
    def com_show(self):
        self.root5=Tk()
        self.root5.title("Company Database")
        self.root5.geometry('400x400')
        self.l8=Label(self.root5,text="CpRegNo  ")
        self.l8.grid(row=0,column=0)
        self.l9=Label(self.root5,text="CpName  ")
        self.l9.grid(row=0,column=1)
        self.l10=Label(self.root5,text="CpDomain  ")
